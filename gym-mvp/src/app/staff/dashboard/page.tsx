...
export default function StaffDashboard() {
  ...
  const handleSendWhatsApp = (screening: ScreeningState) => {
    if (!screening.finalCategory) {
      alert('Screening not yet completed. Results cannot be sent.');
      return;
    }
    const message = `Hi ${screening.name}! Your health screening (ID: ${screening.screeningId}) is complete.\n\nFinal Category: ${screening.finalCategory}\n\nDetailed results will be shared with you by the gym staff.`;
    const whatsappUrl = `https://wa.me/${screening.whatsappNumber}?text=${encodeURIComponent(message)}`;
