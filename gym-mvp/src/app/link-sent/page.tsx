...
export default function LinkSentPage() {
  ...
  const handleShare = () => {
    const message = `Hi ${state.name}! Your gym health screening (ID: ${state.screeningId}) is ready for Part 2. Please complete the remaining assessments here: ${screeningLink}`;
    const whatsappUrl = `https://wa.me/${state.whatsappNumber}?text=${encodeURIComponent(message)}`;
